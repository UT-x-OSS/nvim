	...
	config = function()
		...
		require("mason-lspconfig").setup({
			ensure_installed = {
				...
				"intelephense", "jdtls", "jsonls", "lua_ls", "phpactor",
				"solargraph", "sourcery", "sqlls", "superhtml", "svelte", "tailwindcss",
				"taplo", "ts_ls", "ts_query_ls", "unocss", "vimls",
		...
		-- Function to be used by multiple LSP servers
		local on_attach = function(client, bufnr)
			map("n", "K", vim.lsp.buf.hover, { buffer = bufnr })
			map("i", "<C-Space>", vim.lsp.buf.completion, { buffer = bufnr })
		end
-- 🔧 Basic servers to setup
local servers = {
  bashls = {},
  clojure_lsp = {},
  clangd = {},
  ...
  html = {}, -- ini sudah di awal
		...
		require 'lspconfig'.jdtls.setup {
			...
			on_attach = function(client, bufnr)
				...
				vim.api.nvim_buf_set_keymap(bufnr, 'i', '<C-Space>',
					'<Cmd>lua vim.lsp.buf.completion()<CR>', { noremap = true, silent = true })
			end,
...
SELECT ${1:*} FROM (
  SELECT ${2:*} FROM ${3:table} WHERE ${4:condition}
		...
		lspconfig.sourcery.setup {
			...
			init_options = {
				token = "", -- visit sourcery and login via github or google to acces sourcery token and paste to "here"
			},
		}
		...
		lspconfig.phpactor.setup {
			on_attach = function(client, bufnr)
				...
				buf_set_keymap('n', '<leader>db', '<cmd>lua require("phpactor").query_database()<CR>',
					opts)                                                 -- Query database langsung
				buf_set_keymap('n', '<leader>ds', '<cmd>lua require("phpactor").show_schema()<CR>', opts) -- Tampilkan skema DB
				buf_set_keymap('n', '<leader>di', '<cmd>lua require("phpactor").inspect_schema()<CR>',
					opts)                                                 -- Inspeksi skema DB
				...
				buf_set_keymap('n', '<leader>tf', '<cmd>lua require("phpactor").run_feature_tests()<CR>',
					opts)                                               -- Jalankan feature test

				-- Keymaps untuk refactoring lanjutan
				buf_set_keymap('n', '<leader>refactor', '<cmd>lua require("phpactor").refactor()<CR>',
					opts) -- Refactor kode PHP
				buf_set_keymap('n', '<leader>ext', '<cmd>lua require("phpactor").extract_method()<CR>',
					opts) -- Ekstrak metode dari fungsi
				...
				buf_set_keymap('n', '<leader>ca',
					'<cmd>lua require("lspsaga.codeaction").code_action()<CR>', opts) -- Code Action
				...
				buf_set_keymap('n', '<leader>wr', '<cmd>lua vim.lsp.buf.remove_workspace_folder()<CR>',
					opts)                                                  -- Hapus folder workspace
				buf_set_keymap('n', '<leader>wl',
					'<cmd>lua print(vim.inspect(vim.lsp.buf.list_workspace_folders()))<CR>', opts) -- List folder workspace
			end,
